...
def scan(rule,path):
    for file in os.listdir(path):
        mapth = os.path.join(path,file)
        print(malpath + "\\"+file,'rb')
        fp = open(malpath+"\\"+file,'rb')
        matches =rule.matches(data = fp.read())
        if len(matches) > 0:
            print(file,matches)


if __name__ == '__main__':
    rulepath = "E:\\pycode\\bad_code\\rules"   # yara规则目录
    malpath = "E:\\pycode\\bad_code\\simples"    # simple目录
    # yara规则编译函数调用
...
# 2.机器学习与恶意样本
趋势：
现在的产品越来越喜欢用机器学习这类的大小来做恶意代码的识别

产品结构
很多很多样本----特征工程---好（0）坏（1） -----向量
1.exe
（
virtuallco                  1
rtlmovememry          2
...
```
pe.DIRECTORY_ENTRY_IMPORT[0].imports[0].name
```
